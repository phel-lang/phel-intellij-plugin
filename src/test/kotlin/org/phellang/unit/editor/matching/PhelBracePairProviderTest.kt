...
class PhelBracePairProviderTest {
    ...
    fun `getBracePairs should return all supported pairs`() {
        ...
        assertNotNull(readerCondPair)
        assertEquals(PhelTypes.PAREN2, readerCondPair!!.rightBraceType)
        assertFalse(readerCondPair.isStructural)

        // Check reader conditional splice #?@(
        val readerCondSplicePair = pairs.find { it.leftBraceType == PhelTypes.READER_COND_SPLICE }
        assertNotNull(readerCondSplicePair)
        assertEquals(PhelTypes.PAREN2, readerCondSplicePair!!.rightBraceType)
        assertFalse(readerCondSplicePair.isStructural)
    ...
    fun `isClosingBrace should correctly identify closing braces`(
        elementType: com.intellij.psi.tree.IElementType, expected: Boolean
    ) {
